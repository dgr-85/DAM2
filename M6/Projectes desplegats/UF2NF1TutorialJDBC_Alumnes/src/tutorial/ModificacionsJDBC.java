...
public class ModificacionsJDBC {
	...
	public void afegirEmpleat(Connection connexio) {
		...
		try {
			...
			sqlPreparat.setInt(8, Integer.parseInt(demanarDades("Introdueix el codi del seu departament.")));

			int rs = sqlPreparat.executeUpdate();
			if (rs > 0) {
			...
			if(e.getErrorCode()==1452) {
				System.out.println("El codi del cap de l'empleat no existeix.");
			}
			e.printStackTrace();
