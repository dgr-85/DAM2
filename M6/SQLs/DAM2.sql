...
/*Cuando la cagas con el update y actualizas SALA con los valores de HOSPITAL en lugar de hacerlo al revés.
...
SELECT HOSPITAL_COD,SALA_COD,EMPLEAT_NO,COGNOM FROM PLANTILLA WHERE UPPER(TORN)<>'N';-- <> és igual que !=
--------------- 3 formes de la mateixa consulta
SELECT * FROM MALALT WHERE TO_CHAR(DATA_NAIX,'YYYY') LIKE '1960';
SELECT * FROM MALALT WHERE DATA_NAIX LIKE '%/60';
SELECT * FROM MALALT WHERE EXTRACT(YEAR FROM DATA_NAIX) = 1960;
---------------
SELECT * FROM MALALT WHERE EXTRACT(YEAR FROM DATA_NAIX) > 1960;
SELECT * FROM DOCTOR WHERE HOSPITAL_COD=22;
SELECT I.HOSPITAL_COD,I.SALA_COD,SEXE,COUNT(*) FROM INGRESSOS I JOIN MALALT M ON I.INSCRIPCIO=M.INSCRIPCIO GROUP BY I.SALA_COD;
--------------- 2 formes de la mateixa consulta
SELECT INSCRIPCIO,COGNOM FROM MALALT WHERE INSCRIPCIO NOT IN(SELECT INSCRIPCIO FROM INGRESSOS);
SELECT M.INSCRIPCIO,M.COGNOM FROM MALALT M LEFT JOIN INGRESSOS I ON M.INSCRIPCIO=I.INSCRIPCIO WHERE I.INSCRIPCIO IS NULL;
---------------
SELECT H.HOSPITAL_COD,NOM,QTAT_LLITS/COUNT(EMPLEAT_NO) FROM HOSPITAL H LEFT JOIN PLANTILLA P ON H.HOSPITAL_COD=P.HOSPITAL_COD
GROUP BY H.HOSPITAL_COD,NOM,QTAT_LLITS ORDER BY 3 DESC; -- order by 3 = ordena per la 3a columna del select
SELECT HOSPITAL_COD,NOM FROM HOSPITAL WHERE HOSPITAL_COD NOT IN(SELECT HOSPITAL_COD FROM DOCTOR WHERE UPPER(ESPECIALITAT) LIKE 'CARDIOLOGIA');