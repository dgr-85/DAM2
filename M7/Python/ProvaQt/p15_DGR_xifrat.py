...
class Xifrador(QMainWindow):
    ...
    def __init__(self):
        ...
        self.bt_xifrar.clicked.connect(lambda: self.codificar(self.te_desxifrat.toPlainText(), self.sb_clau.value(), True))
        self.bt_desxifrar.clicked.connect(lambda:
            self.codificar(self.te_xifrat.toPlainText(), self.sb_clau.value(), False))
        self.show()

    def codificar(self, txt, clau, xifrar):
        if not xifrar:
            clau = -clau
        txt_cod = ''
        for lletra in txt:
